...
#-- Block Name       :    snowflake_ranged_table_restart
#-- Description      :    Restart all held batches in a selected range table
#-- Author           :    WhereScape
#--==============================================================================
#-- Notes / History
...
try {
  ...
  if((Get-Process -Id $caller).Name -eq "WslSched") {
    ...
    $MyForm.Text = "Range Table Restart"
    ...
    $MyLabel.Text = "Please select the ranged table to restart:"
...
"@
      ...
      $status = 1
      $return_Msg = "Future ranges ($rowCount) restarted for $SelectedMETable via range table $rangeSchema.$rangeName."
    }
    else {
      $status = -1
